...
## 🚨 **Phase 0: 긴급 수정 사항** `4/4 (100%)`

### 📁 즉시 해결 필요
- [x] **0.1** TASKS.md 파일을 루트에 생성
- [x] **0.2** shared/solar-terms.ts LSP 에러 9개 즉시 수정 (중복 export 문제)  
- [x] **0.3** 2025년 전체 24절기 데이터 긴급 추가 (한국천문연구원 기준)
- [x] **0.4** 하드코딩된 2024년을 동적 연도로 변경 (pdf-generator.ts, home.tsx)
...
### 🔍 현재 상태 분석
- [x] **1.1** shared/schema.ts의 SajuData 인터페이스 상세 분석
- [x] **1.2** premium-calculator.ts의 PremiumSajuAnalysis 구조 분석
- [x] **1.3** 두 데이터 구조 간 차이점 매핑 테이블 작성
- [x] **1.4** 손실되는 데이터 필드 식별 및 보존 방법 결정

### 🔌 어댑터 패턴 구현
- [x] **1.5** `client/src/lib/saju-adapter.ts` 파일 생성
- [x] **1.6** `adaptPremiumToBasic()` 함수 구현 - 기본 호환성
- [x] **1.7** `preservePremiumExtras()` 함수 구현 - 프리미엄 데이터 보존
- [x] **1.8** 신살 데이터 JSON 직렬화 로직 구현
- [x] **1.9** 음력 데이터 포함 로직 구현

### 🧪 어댑터 테스트
- [x] **1.10** `__tests__/saju-adapter.test.ts` 파일 생성
- [x] **1.11** 1989-10-06 12:56 골든 케이스 테스트 작성
- [x] **1.12** 윤달 케이스 테스트 작성 (1984-10-23)
- [x] **1.13** 절기 경계선 케이스 테스트 작성 (입춘 전후)
- [x] **1.14** 자정/정오 경계 케이스 테스트 작성
...
**⚡ 다음 우선순위: Task 0.2 - LSP 에러 9개 즉시 수정**